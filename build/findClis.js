import { canExecute, exploreOperationFolders, fs, readJsonFile, getFileFirstLine, } from "from-anywhere/node";
import path from "path";
import { notEmpty } from "from-anywhere";
export const findClis = async (context) => {
    const { absoluteFolderPath } = context;
    const operationFolders = await exploreOperationFolders({
        basePath: absoluteFolderPath,
    });
    const bins = (await Promise.all(operationFolders.map(async (absoluteFolderPath) => {
        const packageJson = await readJsonFile(path.join(absoluteFolderPath, "package.json"));
    ...
    const entireBinList = (await Promise.all(bins.map(async (item) => {
        const binList = await Promise.all(Object.keys(item.bin).map(async (command) => {
            const binLocation = path.join(item.absoluteFolderPath, item.bin[command]);
    ...
    const badClis = entireBinList
        .filter((x) => !x.isCool)
        .map((x) => {
        const { absoluteFolderPath, command, executable, exists, isJs, isNodeEnv, } = x;
        return {
            absoluteFolderPath,
            command,
            executable,
...
//# sourceMappingURL=findClis.js.map